...
class BDCN(nn.Module):
    def __init__(self, pretrain=None, logger=None, rate=4):
        ...
        # customize 2d
        # self.features.conv1_1 = nn.Conv2d(4, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        # self.features.conv1_1  = customize_2d_conv()
...
class pixel_connect_network(nn.Module):
    def __init__(self, bdcn, unet):
        super(pixel_connect_network, self).__init__()
        self.bdcn = bdcn
        self.unet = unet

    def forward(self, x):
        y = self.bdcn(x)
        bdcn_out = nn.ReLU()(y[-1])
        connect_prob = self.unet(bdcn_out)
        return [y, connect_prob]

def image_gradient(input_image):
    # The connectivity is happen when gradient is small -> more sticky
    input_image = input_image.detach().cpu().numpy().squeeze()
    x_dir, y_dir = np.gradient(input_image)
    img_grad = np.dstack((x_dir, y_dir))
    img_grad = torch.from_numpy(img_grad).float().cuda()
    img_grad = torch.unsqueeze(img_grad.permute(2, 0, 1), 0)
    return img_grad
